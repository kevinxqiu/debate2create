...
class Halfcheetah(PipelineEnv):
  ...
  def __init__(
    ...
    if xml_path is None:
        # Fallback to default Brax asset if no xml_path provided
        xml_path = epath.resource_path('brax') / 'envs/assets/half_cheetah.xml'
  ...
  def reset(self, rng: jax.Array) -> State:
    ...
    info = {
        ...
        'prev_x_velocity': jp.array(0.0, dtype=obs.dtype),
        'reward_components': {},
    }
  ...
  def step(self, state: State, action: jax.Array) -> State:
    ...
    if getattr(self, "_use_custom_reward", False):
      ...
      prev_x_vel = state.info.get('prev_x_velocity', jp.array(0.0, dtype=obs.dtype)) if hasattr(state, 'info') else jp.array(0.0, dtype=obs.dtype)
      ...
      new_info['prev_x_velocity'] = x_velocity
    else:
      reward = base_reward
      new_info = dict(state.info)
